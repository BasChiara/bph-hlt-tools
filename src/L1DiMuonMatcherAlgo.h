#ifndef MuonAnalysis_MuonAssociators_interface_L1DiMuonMatcherAlgo_h
#define MuonAnalysis_MuonAssociators_interface_L1DiMuonMatcherAlgo_h
...
#include "MuonAnalysis/MuonAssociators/src/PropagateToMuon.cc"

template <edm::Transition Tr>
class L1DiMuonMatcherAlgoT {
public:
  explicit L1DiMuonMatcherAlgoT(const edm::ParameterSet &iConfig, edm::ConsumesCollector);
  ~L1DiMuonMatcherAlgoT() = default;
...
template <edm::Transition Tr>
template <typename Collection, typename Selector>
int L1DiMuonMatcherAlgoT<Tr>::matchGeneric(TrajectoryStateOnSurface &propagated,
...
template <edm::Transition Tr>
L1DiMuonMatcherAlgoT<Tr>::L1DiMuonMatcherAlgoT(const edm::ParameterSet &iConfig, edm::ConsumesCollector iCollector)
  ...
  if (reqPhi + reqEta + reqPt + reqQual > 1)
    throw cms::Exception("Configuration")
        << "L1DiMuonMatcherAlgoT: Can't set more than one 'sortBy<XXX>' parameter to True.\n";
  if (sortBy == "deltaPhi") {
    if (reqEta || reqPt || reqQual)
      throw cms::Exception("Configuration") << "L1DiMuonMatcherAlgoT: Can't set sortBy = 'deltaPhi' and set also another "
  ...
  if (sortBy == "deltaEta") {
    if (reqPhi || reqPt || reqQual)
      throw cms::Exception("Configuration") << "L1DiMuonMatcherAlgoT: Can't set sortBy = 'deltaEta' and set also another "
                                               "'sortBy<XXX>' parameter to True.\n";
    else
      reqEta = true;
  }
  if (sortBy == "pt") {
    if (reqPhi || reqEta || reqQual)
      throw cms::Exception("Configuration")
          << "L1DiMuonMatcherAlgoT: Can't set sortBy = 'pt' and set also another 'sortBy<XXX>' parameter to True.\n";
    else
      reqPt = true;
  }
  if (sortBy == "quality") {
    if (reqPhi || reqEta || reqPt)
      throw cms::Exception("Configuration")
          << "L1DiMuonMatcherAlgoT: Can't set sortBy = 'quality' and set also another 'sortBy<XXX>' parameter to True.\n";
    else
      reqQual = true;
  }
  if (sortBy == "deltaR") {
    if (reqPhi || reqEta || reqPt || reqQual)
      throw cms::Exception("Configuration")
          << "L1DiMuonMatcherAlgoT: Can't set sortBy = 'deltaR' and set also another 'sortBy<XXX>' parameter to True.\n";
  }
...
template <edm::Transition Tr>
void L1DiMuonMatcherAlgoT<Tr>::init(const edm::EventSetup &iSetup) {
  prop_ = propSetup_.init(iSetup);
}

template <edm::Transition Tr>
bool L1DiMuonMatcherAlgoT<Tr>::match(TrajectoryStateOnSurface &propagated,
                                   const l1extra::L1MuonParticle &l1,
                                   float &deltaR,
                                   float &deltaPhi) const {
...
template <edm::Transition Tr>
int L1DiMuonMatcherAlgoT<Tr>::match(TrajectoryStateOnSurface &propagated,
...
using L1DiMuonMatcherAlgo = L1DiMuonMatcherAlgoT<edm::Transition::Event>;
