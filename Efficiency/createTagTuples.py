...
my_parser.add_argument('--data_path',
                    ...
                    help='folder path containing the input files')
my_parser.add_argument('--output_path',
                    type = str,
                    default='/eos/cms/store/group/phys_bphys/cbasile/BPH_TagTuples/',
...
dataset_info = data_path.split('/')
dataset_wrapper = {
    'dataset': dataset_info[indx-1].replace('*',''),
    'sample': dataset_info[indx].replace('*',''),
    'year': args.year if args.year else dataset_info[indx].split('-')[1],
    'version': dataset_info[-2].split('-')[-1],
}
print(dataset_wrapper)
data_name=f'{name+"_" if name else ""}{dataset_wrapper["dataset"]}_{dataset_wrapper["year"]}_{dataset_wrapper["sample"]}_{tagPath}{"_"+dataset_wrapper["version"] if dataset_wrapper["version"] else ""}'
print(data_name)
