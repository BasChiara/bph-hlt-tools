...
if __name__== '__main__':
    ...
    parser.add_argument('-c', '--config', type=str,
                        help="Json file with selection configuration")
    ...
    # tag & probe selection from config
    if args.config:
        with open(args.config) as f:
            settings = json.load(f)
        f.close()
        denQuery = ' & '.join(settings.get('tag_selection'))
        numQuery = ' & '.join(settings.get('probe_selection'))
    else:
